...
export const NotificationCenterModal: React.FC<NotificationCenterModalProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'scholarship' | 'system_update' | 'unread' | 'settings'>('all');
            ...
            {[
              ...
              return (
                ...
                >
                  ...
                  {tab.count !== undefined && (
                    <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono ${isActive ? 'bg-amber-400 text-slate-950 font-black' : 'bg-[#1C162A] text-gray-400'}`}>
                      {tab.count}
                    </span>
                  )}
                </button>
          ...
          {activeFilter === 'settings' ? (
            ...
            <div className="py-16 text-center space-y-3 bg-[#0B0715] border border-[#241A37] rounded-3xl p-8">
              ...
              <p className="text-xs text-gray-500 max-w-sm mx-auto">
                {language === 'ti'
                  ? 'ሓደሽቲ ዕድላት ስኮላርሺፕ ወይ ናይ ስርዓት ዜናታት ምስ ዝመጽእ ኣብዚ ክዝርዘር እዩ።'
                  : 'New scholarship opportunities and system upgrade bulletins will appear here.'}
              </p>
            </div>
          ) : (
            ...
            filteredNotifications.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {isUnread && (
                    <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-gradient-to-b from-amber-400 via-[#F3E5AB] to-amber-500" />
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs gap-2">
                      <div className="flex items-center space-x-2 flex-wrap">
                        <span className={`p-1.5 rounded-lg ${isScholarship ? 'bg-amber-500/20 text-amber-300' : 'bg-sky-500/20 text-sky-300'}`}>
                          {isScholarship ? <GraduationCap className="w-4 h-4" /> : <RefreshCw className="w-4 h-4" />}
                        </span>
                        
                        <span className="text-[11px] font-mono font-bold uppercase tracking-wider text-amber-300">
                          {isScholarship ? (language === 'ti' ? 'ዕድል ስኮላርሺፕ' : 'Scholarship Opportunity') : (language === 'ti' ? 'ናይ ስርዓት ዜና' : 'System Update')}
                        </span>

                        {item.badgeText && (
                          <span className="px-2 py-0.2 rounded-full text-[9px] font-bold uppercase bg-amber-500/20 text-amber-200 border border-amber-400/40">
                            {item.badgeText}
                          </span>
                        )}
                    ...
                    <div className="space-y-1">
                      {/* Tigrinya Title */}
                      <h4 className="text-sm font-bold text-white group-hover:text-[#F3E5AB] transition-colors leading-snug">
                        {item.titleTi}
                      </h4>

                      {/* English Title */}
                      {(preferences.preferredLanguage === 'bilingual' || preferences.preferredLanguage === 'en') && (
                        <div className="text-xs font-semibold text-amber-200/80 font-sans">
                          {item.titleEn}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="text-xs text-gray-300 leading-relaxed font-sans space-y-1 bg-[#090612] p-2.5 rounded-xl border border-[#201535]">
                      {preferences.preferredLanguage !== 'en' && (
                        <p className="text-slate-200">{item.bodyTi}</p>
                      )}
                      {(preferences.preferredLanguage === 'bilingual' || preferences.preferredLanguage === 'en') && (
                  ...
                  <div className="pt-2 border-t border-[#201535] flex items-center justify-between gap-2 flex-wrap" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-[#1A132C] hover:bg-[#251B3E] text-gray-300 hover:text-white rounded-lg border border-[#3A2A54] text-[10.5px] font-bold transition-all flex items-center space-x-1 cursor-pointer"
                      >
                        <CheckCircle2 className={`w-3 h-3 ${isUnread ? 'text-amber-400' : 'text-gray-500'}`} />
                    ...
                    <button
                      onClick={() => handleItemAction(item)}
                      className="px-3.5 py-1.5 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-black rounded-xl text-xs transition-all flex items-center space-x-1 shadow-md shadow-amber-500/20 active:scale-95 cursor-pointer"
                    >
